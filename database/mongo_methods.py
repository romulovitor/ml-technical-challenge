...
class MongoAcess:
    ...
    def insert_mongo(self, characteristic, src):
        """
        ...
        :param src indicate if wich database conection we should use
        :return: print the sucess or failure of process
        """

        if src:
            self.string_conection = 'mongodb://' + self.db_user + ':' + self.db_pass + '@localhost:' + self.port
